		...
		if (global == null) {
			try {
				...
				if(Dungeon.isTutorial){
					...
					input = Game.instance.openFileInput( Dungeon.template.name + BADGES_FILE );
				}
				else{
		...
		if (saveNeeded) {
			...
			try {
				...
				if(Dungeon.isTutorial){
					...
					output = Game.instance.openFileOutput(Dungeon.template.name + BADGES_FILE, Game.MODE_PRIVATE );
				}
				else{
